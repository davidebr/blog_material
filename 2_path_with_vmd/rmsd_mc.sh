...
usage()
{
  ...
  echo "./rmsd_mc.sh  -f filename  -r 80 -n 30 -s 1 -m -t 50  -d " 
  echo "   where :"
  echo "      -f filename: the input pdb file from which you want to extract equally spaced frames"
  echo "      -r 80: reparametrize the path in 80 frames    "
  echo "      -m  : use monte carlo optimization     "
  echo "      -s 1 : optimize with a monte carlo step size of 1. smaller steps allow for a slower optimization but take longer     "
  echo "      -t  50 : read input file with a stride of 50    "
  echo "      -d  : dump matrix and diagonal elements    "
  echo "          "
  echo " e.g " 
  echo " ./rmsd_mc.sh -f file.dat -r 80  -n 30 -s 1  -d "
  echo " -d just dump the matrix "
  echo " ./rmsd_mc.sh -f file.dat -r 80  -n 30 -s 1 -m -t 50 -d "
  echo "create also a file REPARAM.pdb which contains 80 reparametrized frames "
  echo "created with 30 steps (-n 30) with increase of step 1 ( -s 1 ) with mc ( -m ) and a stride of 50 (-t 1) " 
...
do
	case $OPTION in
	     ...
	     f)
	             filename=$OPTARG
	            echo "reparametrizing file $filename "		
	     ;;
	     r)
		    ndiv=$OPTARG
...
proc harmonic_potential_and_force { molid reference_coords reference_deltas  myindex myfactor } {
	...
	set delta [ test_rescaling  $molid $reference_coords $reference_deltas  $myindex $myfactor ] 
	set harm 0. ; 
...
proc dump_frames { molid reference_coords reference_deltas  myindex myfactor } {
   set mydir [ exec  pwd  ];
   exec rm -rf REPARAM.pdb .ff_* 
   for { set i  0 } { $i < [llength $myindex ] } { incr i } {
	...
	set newcoord [ newscaledcoords [ lindex $reference_coords $id ]  [ lindex $reference_deltas $id ] [ lindex $myfactor $i ] ] 
   	...
   	exec cat .ff_$i.pdb >>REPARAM.pdb
...
if { $mc == 1} {
	...
	dump_frames 0 $reference_coords $reference_deltas  $myindex $myfactor
} else {
	for { set j 0  } { $j < $nsteps } { incr j } {
		# calculate distance
		set newdelta [ test_rescaling 0 $reference_coords $reference_deltas  $myindex $myfactor ] ;  puts -nonewline  ""
		# save matrix
...
quit
EOF
sed  s/I_NDIV/$ndiv/ .l.tcl | sed s/I_STEPSIZE/$stepsize/ | sed s/I_NSTEPS/$nsteps/ | sed s/I_MC/$mc/ | sed s/I_STRIDE/$stride/ | sed s/I_FILENAME/$filename/ | sed s/I_JUSTDUMP/$justdump/ > .m.tcl; mv .m.tcl .l.tcl 
...
close FILE;
EOF
chmod +x .p.pl ; ./.p.pl $filename REPARAM.pdb |grep -v CRYS >.l ; mv .l REPARAM.pdb ; rm -rf .p.pl
fi
